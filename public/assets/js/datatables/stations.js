...
var datatable;
// Class definition
var KTDatatablesStations = (function () {
    let dbTable = "stations";
    let prefix = "admin";
    ...
    var initDatatable = function () {
        datatable = $("#kt_datatable").DataTable({
            ...
            columns: [
                ...
                { data: "is_active" }, // Is Active
                { data: "established_at" }, // Established at
...

                {
                    ...
                    render: function (data, type, row) {
                        return `
                            <div>
                                <div class="d-flex flex-column justify-content-center">
                                    <a href="javascript:;" class="mb-1 text-gray-800 text-hover-primary">${row.established_at}</a>
                                </div>
                            </div>`;
                    },
                },
    ...
    var handleEditRows = () => {
        ...
        editButtons.forEach((btn) => {
            btn.addEventListener("click", function (e) {
                ...
                if (!data) return; // حماية لو البيانات مش موجودة

                // تحديث عنوان المودال
                $("#form_title").text(__("Edit Station"));

                // ملء القيم مع التحقق من وجودها
                $("#name_ar_inp").val(data.name_ar || "");
                $("#address_inp").val(data.address || "");
                $("#description_inp").val(data.description || "");
                $("#plate_number_inp").val(data.plate_number || "");
                $("#is_active_inp").prop("checked", data.is_active == 1);

                // تواريخ
                $("#kt_datepicker_1_inp").val(data.established_at || "");
                $("#last_maintenance_inp").val(data.last_maintenance || "");
                // عند فتح المودال
                if (data.full_image_path) {
                    ...
                    $(".image-input-wrapper").css(
                        "background-image",
                        "url('/assets/media/svg/avatars/default.svg')"
                    );
                }
                ...
                if (data.status_id)
                    $("#status_id_inp").val(data.status_id).trigger("change");

                // تحديث عنوان الفورم والـ method
                $("#crud_form").attr(
                    "action",
