var KTChartsWidget5 = (function () {
    ...
    var initChart = function (chart) {
        ...
        if (
            !element ||
            typeof window.dashboardAmana === "undefined" ||
            !window.dashboardAmana.employeesByNeighborhood
        ) {
            return;
        }
        ...
        var data = window.dashboardAmana.employeesByNeighborhood;
            ...
            xaxis: {
                ...
                labels: {
                    formatter: function (val) {
                        // val هنا هو القيمة الرقمية على المحور (عدد الموظفين)
                        return val;
                    },
                    style: {
...
document.addEventListener("DOMContentLoaded", function () {
    if (
        typeof KTChartsWidget5 !== "undefined" &&
        typeof KTChartsWidget5.init === "function"
    ) {
...
var DashboardAmanaCharts = (function () {
    function initMissionsSimple() {
        ...
        if (!window.dashboardAmana || !window.dashboardAmana.missionsSimple) {
            el.innerHTML =
                '<div class="d-flex align-items-center justify-content-center h-100 min-h-300px"><div class="text-center"><i class="ki-outline ki-information-5 fs-3x text-muted mb-3"></i><div class="text-gray-500 fw-semibold">' +
    ...
    function initTopDistrictsChart() {
        ...
        var labels = [
            // cfg.labels[maxIndex] + " - " + __("People"),
            // cfg.labels[maxIndex] + " - " + __("Companies"),
            cfg.labels[maxIndex] + " - " + __("Extinguishers"),
        ];

        var colors = ["#f59e0b", "#198754", "#ffc107"]; // الأزرق، الأخضر، الأصفر
    ...
    function initExtinguishersStatus() {
        ...
        if (
            !el ||
            typeof ApexCharts === "undefined" ||
            !window.dashboardAmana ||
            !window.dashboardAmana.extinguishersStatus
        ) {
            return;
        }
    ...
    function initExtinguishersByCompany() {
        ...
        if (
            ...
            typeof ApexCharts === "undefined" ||
            !window.dashboardAmana ||
            !window.dashboardAmana.extinguishersByCompany
        ) {
            return;
        }
