"use strict";

function truncateText(text, maxLength = 50) {
    if (!text) return "";
    if (text.length > maxLength) {
        let visibleText = text.substr(0, maxLength);
        let hiddenText = text.substr(maxLength);

        return `
            <span class="short-text">${visibleText}</span>
            <span class="more-text" style="display:none">${hiddenText}</span>
            <span class="more-link text-primary" style="cursor:pointer" onclick="toggleText(this)"> ...</span>
        `;
    }
    return text;
}

function toggleText(element) {
    const moreText = element.previousElementSibling;
    const isHidden = moreText.style.display === "none";

    if (isHidden) {
        moreText.style.display = "inline";
        element.innerHTML = "..";
    } else {
        moreText.style.display = "none";
        element.innerHTML = " ......";
    }
}

var datatable;
// Class definition
var KTDatatablesCompanies = (function () {
    let dbTable = "abstractes";
    let prefix = userType;

    // Private functions
    var initDatatable = function () {
        datatable = $("#kt_datatable").DataTable({
            language: language,
            searchDelay: searchDelay,
            serverSide: serverSide,
            order: [],
            stateSave: saveState,
            select: {
                style: "multi",
                selector: 'td:first-child input[type="checkbox"]',
                className: "row-selected",
            },
            ajax: {
                url: `/dashboard/${prefix}/${dbTable}`,
                data: function (d) {
                    d.status_id = $("#status_filter").val();
                    d.contract_id = $("#contractor_filter").val();
                },
            },
            columns: [
                { data: "id" },
                { data: "title" },
                { data: "description" },
                { data: "upload" },
                { data: "contract_id" },
                { data: "status_id" },
                { data: "created_at" },
                { data: "type" },
                { data: "approved_by" },
                { data: null },
            ],
            columnDefs: [
                {
                    targets: 0,
                    orderable: false,
                    render: function (data) {
                        return `
                            <div class="form-check form-check-sm form-check-custom form-check-solid ">
                                <input class="form-check-input" type="checkbox" value="${data}" />
                            </div>`;
                    },
                },
                {
                    targets: 1, // ✅ contract name
                    render: function (data, type, row) {
                        return `
                            <div>
                                <div class="d-flex flex-column justify-content-center">
                                    <a href="javascript:;" class=" text-gray-800 text-hover-primary">${
                                        row.title ?? "--"
                                    }</a>
                                </div>
                            </div>`;
                    },
                },

                {
                    targets: 2,
                    render: function (data, type, row) {
                        return `
                            <div>
                                <div class="d-flex flex-column justify-content-center">
                                    <a href="javascript:;" class=" text-gray-800 text-hover-primary">${truncateText(
                                        row.description
                                    )}</a>
                                </div>
                            </div>`;
                    },
                },
                {
                    targets: 3, // ✅ upload
                    render: function (data, type, row) {
                        if (!row.upload) {
                            return `<span class="text-muted">${__(
                                "No file uploaded"
                            )}</span>`;
                        }
                        const fileUrl = `/storage/abstractes/${row.upload}`;
                        return `
                            <div class="d-flex justify-content-center">
                                <a href="${fileUrl}" target="_blank" class="btn btn-sm btn-light-primary">
                                    <i class="ki-outline ki-document fs-3"></i>
                                </a>
                            </div>`;
                    },
                },
                {
                    targets: 4, // ✅ contract name
                    render: function (data, type, row) {
                        return `
                            <div>
                                <div class="d-flex flex-column justify-content-center">
                                    <a href="javascript:;" class=" text-gray-800 text-hover-primary">${
                                        row.contract?.name_ar ?? "--"
                                    }</a>
                                </div>
                            </div>`;
                    },
                },

                {
                    targets: 5,
                    orderable: false,
                    render: (data, type, row) => {
                        if (!row.status) return "--";
                        return `
                            <span class="badge"
                                style="background-color:${
                                    row.status.color ?? "#ccc"
                                }; color:#fff;">
                                ${row.status.name}
                            </span>`;
                    },
                },

                {
                    targets: 6, // created_at
                    render: function (data, type, row) {
                        return `
                            <div>
                                <div class="d-flex flex-column justify-content-center">
                                    <a href="javascript:;" class=" text-gray-800 text-hover-primary">${row.created_at}</a>
                                </div>
                            </div>`;
                    },
                },
                {
                    targets: 7, // ✅ type
                    render: function (data, type, row) {
                        return `
                            <div>
                                <div class="d-flex flex-column justify-content-center">
                                    <a href="javascript:;" class=" text-gray-800 text-hover-primary">${__(
                                        row.type ?? "--"
                                    )}</a>
                                </div>
                            </div>`;
                    },
                },

                {
                    targets: 8,
                    render: function (data, type, row) {
                        if (row.is_approved) {
                            return `
                                <div class="d-flex flex-column justify-content-center">
                                    <span class="text-gray-800 fw-bold">
                                        ${row.approved?.name ?? "--"}
                                    </span>
                                </div>`;
                        }
                        return `
                            <div class="d-flex flex-column justify-content-center">
                                <span class="b-1 text-gray-800">${__(
                                    "Not approved"
                                )}</span>
                            </div>`;
                    },
                },
                {
                    targets: -1,
                    data: null,
                    orderable: false,
                    render: function (data, type, row) {
                        let approveHtml = "";
                        let editHtml = "";
                        let deleteHtml = "";

                        if (
                            !row.is_approved &&
                            (prefix === "admin" || prefix === "amana")
                        ) {
                            approveHtml = `
                                <div class="menu-item px-3">
                                    <a href="javascript:;" class="menu-link px-3 approve-btn" data-id="${
                                        row.id
                                    }">
                                        ${__("Approve")}
                                    </a>
                                </div>
                            `;
                        }

                        // زر التعديل يظهر فقط إذا لم تتم الموافقة
                        if (
                            !row.is_approved &&
                            (prefix === "admin" ||
                                prefix === "contractor" ||
                                prefix === "consultant")
                        ) {
                            editHtml = `
                                <div class="menu-item px-3">
                                    <a href="javascript:;" class="menu-link px-3" data-kt-docs-table-filter="edit_row">
                                        ${__("Edit")}
                                    </a>
                                </div>
                            `;
                        }

                        // لو Admin → يظهر الحذف دائماً

                        if (
                            prefix === "admin" ||
                            (!row.is_approved &&
                                (prefix === "contractor" ||
                                    prefix === "consultant"))
                        ) {
                            deleteHtml = `
                                    <div class="menu-item px-3">
                                        <a href="#" class="menu-link px-3" data-kt-docs-table-filter="delete_row">
                                            ${__("Delete")}
                                        </a>
                                    </div>
                                `;
                        }

                        return `
                            <div>
                              <a href="#" class="btn btn-light btn-active-light-primary btn-sm"
                   data-kt-menu-trigger="click"
                   data-kt-menu-placement="bottom-end"
                   data-kt-menu-flip="top-end">
                    <span class="svg-icon svg-icon-dark svg-icon-1 m-0">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                             xmlns="http://www.w3.org/2000/svg">
                            <path opacity="0.3"
                                  d="M22.1 11.5V12.6C22.1 13.2 21.7 13.6 21.2 13.7L19.9 13.9C19.7 14.7 19.4 15.5 18.9 16.2L19.7 17.2999C20 17.6999 20 18.3999 19.6 18.7999L18.8 19.6C18.4 20 17.8 20 17.3 19.7L16.2 18.9C15.5 19.3 14.7 19.7 13.9 19.9L13.7 21.2C13.6 21.7 13.1 22.1 12.6 22.1H11.5C10.9 22.1 10.5 21.7 10.4 21.2L10.2 19.9C9.4 19.7 8.6 19.4 7.9 18.9L6.8 19.7C6.4 20 5.7 20 5.3 19.6L4.5 18.7999C4.1 18.3999 4.1 17.7999 4.4 17.2999L5.2 16.2C4.8 15.5 4.4 14.7 4.2 13.9L2.9 13.7C2.4 13.6 2 13.1 2 12.6V11.5C2 10.9 2.4 10.5 2.9 10.4L4.2 10.2C4.4 9.39995 4.7 8.60002 5.2 7.90002L4.4 6.79993C4.1 6.39993 4.1 5.69993 4.5 5.29993L5.3 4.5C5.7 4.1 6.3 4.10002 6.8 4.40002L7.9 5.19995C8.6 4.79995 9.4 4.39995 10.2 4.19995L10.4 2.90002C10.5 2.40002 11 2 11.5 2H12.6C13.2 2 13.6 2.40002 13.7 2.90002L13.9 4.19995C14.7 4.39995 15.5 4.69995 16.2 5.19995L17.3 4.40002C17.7 4.10002 18.4 4.1 18.8 4.5L19.6 5.29993C20 5.69993 20 6.29993 19.7 6.79993L18.9 7.90002C19.3 8.60002 19.7 9.39995 19.9 10.2L21.2 10.4C21.7 10.5 22.1 11 22.1 11.5ZM12.1 8.59998C10.2 8.59998 8.6 10.2 8.6 12.1C8.6 14 10.2 15.6 12.1 15.6C14 15.6 15.6 14 15.6 12.1C15.6 10.2 14 8.59998 12.1 8.59998Z"
                                  fill="currentColor"/>
                            <path d="M17.1 12.1C17.1 14.9 14.9 17.1 12.1 17.1C9.30001 17.1 7.10001 14.9 7.10001 12.1C7.10001 9.29998 9.30001 7.09998 12.1 7.09998C14.9 7.09998 17.1 9.29998 17.1 12.1ZM12.1 10.1C11 10.1 10.1 11 10.1 12.1C10.1 13.2 11 14.1 12.1 14.1C13.2 14.1 14.1 13.2 14.1 12.1C14.1 11 13.2 10.1 12.1 10.1Z"
                                  fill="currentColor"/>
                        </svg>
                    </span>
                </a>

                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600
                                            menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">

                                    ${editHtml}
                                    <div class="menu-item px-3">
                                        <a href="/dashboard/${prefix}/${dbTable}/${
                            data.id
                        }" class="menu-link px-3 show_button">
                                            ${__("Show")}
                                        </a>
                                    </div>
                                    ${approveHtml}
                                    ${deleteHtml}
                                </div>
                            </div>
                        `;
                    },
                },
            ],
            createdRow: function (row, data, dataIndex) {},
        });
        $("#contractor_filter, #status_filter").on("change", function () {
            datatable.ajax.reload();
        });

        datatable.on("draw", function () {
            initToggleToolbar();
            toggleToolbars();
            handleEditRows();
            deleteRowWithURL(`/dashboard/${prefix}/${dbTable}/`);
            deleteSelectedRowsWithURL({
                url: `/dashboard/${prefix}/${dbTable}/delete-selected`,
                restoreUrl: `/dashboard/${prefix}/${dbTable}/restore-selected`,
            });
            $("#crud_form")[0].reset(); // يعيد ضبط القيم لكل الحقول في الفورم

            KTMenu.createInstances();
        });
        $("#kt_datatable").on("click", ".approve-btn", function () {
            const button = $(this);
            const itemId = button.data("id");
            button.prop("disabled", true).text("Approving...");

            $.ajax({
                url: `/dashboard/${prefix}/${dbTable}/${itemId}/approve`,
                type: "POST",
                data: { _token: $('meta[name="csrf-token"]').attr("content") },
                success: function (res) {
                    Swal.fire({
                        icon: "success",
                        title: __("Success!"),
                        text: __("Abstracte approved successfully"),
                        timer: 2000,
                        showConfirmButton: false,
                    });
                },
                error: function (err) {
                    Swal.fire({
                        icon: "error",
                        title: __("Error!"),
                        text: __("Something went wrong"),
                        timer: 2000,
                        showConfirmButton: false,
                    });
                    button.prop("disabled", false).text__("Approve");
                },
            });
        });
    };

    var handleEditRows = () => {
        const editButtons = document.querySelectorAll(
            '[data-kt-docs-table-filter="edit_row"]'
        );
        editButtons.forEach((d) => {
            d.addEventListener("click", function (e) {
                e.preventDefault();
                let currentBtnIndex = $(editButtons).index(d);
                let data = datatable.row(currentBtnIndex).data();
                console.log(data.type);

                $("#form_title").text(__("Edit Abstracte"));
                $("#title_inp").val(data.title).trigger("change");
                $("#description_inp").val(data.description).trigger("change");
                // $("#type_id_inp").val(data.type).trigger("change");
                // data.type = "ongoing" أو "final" من DB

                $("#type_id_inp").val(data.type).trigger("change");

                $("#contract_id_inp").val(data.contract_id).trigger("change");

                if (data.upload) {
                    $("#current_file").html(`
                        <a href="${window.location.origin}/storage/abstractes/${data.upload}" target="_blank" class="text-primary">
                            <i class="fa fa-file-pdf"></i> ${data.upload}
                        </a>
                    `);
                } else {
                    $("#current_file").html(
                        `<span class="text-muted">No file uploaded</span>`
                    );
                }

                if (
                    (prefix === "contractor" || prefix === "consultant") &&
                    data.items
                ) {
                    $("#contract_items_container").html("");
                    data.items.forEach((item) => {
                        const checked = item.is_selected ? "checked" : "";
                        $("#contract_items_container").append(`
            <div class="form-check">
                <input class="form-check-input" type="checkbox" name="contract_item_ids[]" value="${item.id}" ${checked}>
                <label class="form-check-label">${item.title}</label>
            </div>
        `);
                    });
                } else {
                    $("#contract_items_container").html(""); // تنظف الحاوية لأي مستخدم غير مسموح له
                }

                // تحديث الفورم
                $("#crud_form").attr(
                    "action",
                    `/dashboard/${prefix}/${dbTable}/${data.id}`
                );

                // تأكيد استخدام PUT
                $("#crud_form").prepend(
                    `<input type="hidden" name="_method" value="PUT">`
                );

                // عرض المودال
                $("#crud_modal").modal("show");
            });
        });
    };

    // Public methods
    return {
        init: function () {
            initDatatable();
            handleSearchDatatable();
            initToggleToolbar();
            handleEditRows();
            deleteRowWithURL(`/dashboard/${prefix}/${dbTable}/`);
            deleteSelectedRowsWithURL({
                url: `/dashboard/${prefix}/${dbTable}/delete-selected`,
                restoreUrl: `/dashboard/${prefix}/${dbTable}/restore-selected`,
            });
        },
    };
})();

// On document ready
KTUtil.onDOMContentLoaded(function () {
    KTDatatablesCompanies.init();
});
