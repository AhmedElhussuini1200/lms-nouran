...
let deleteAlert = function (message = "", confirm = "Yes, Delete !") {
    if (message == "")
        message = `${
...
// تفعيل select2 على كل select عليه data-control="select2"
$(document).ready(function () {
    // استهداف جميع الـ selects اللي فيها data-control="select2"
...
let BlockAlert = function (title, callback) {
    return Swal.fire({
        title: __(title),
        icon: "warning",
        ...
        customClass: {
            confirmButton: "btn fw-bold btn-danger", // Custom style for confirm button
            cancelButton: "btn fw-bold btn-primary", // Custom style for cancel button
        },
        buttonsStyling: false,
...
let toggleBannerVisibility = (bannerName, toggleUrl, isHiding, callback) => {
    console.log(isHiding);
    const confirmText =
        isHiding == 0
            ? __("Are you sure you want to hide this banner?")
            : __("Are you sure you want to unhide this banner?");

    const successText =
        isHiding == 0
            ? `${__("You have hidden")} ${bannerName} ${__("successfully!")}`
            : `${__("You have unhidden")} ${bannerName} ${__("successfully!")}`;
    const confirmButtonText = isHiding == 0 ? "Yes, Hide!" : "Yes, Unhide!";
    deleteAlert(confirmText, confirmButtonText).then(function (result) {
        if (result.value) {
            loadingAlert(
                isHiding == 0 ? __("Hiding now...") : __("Unhiding now...")
            );
            ...
            $.ajax({
                ...
                url: toggleUrl, // this should point to your route for toggling status
                success: (response) => {
                    setTimeout(() => {
                        successAlert(successText).then(function () {
            ...
            errorAlert(__("Action cancelled!"));
        }
    });
};
...
function onImgError(
    image,
    placeholder = "/assets/dashboard/admin/media/placeholders/default.png"
) {
...
//     $(".ajax-form").submit(function (event) {
//         event.preventDefault();
//         submitForm(this); // دالة AJAX لإرسال الفورم
//     });

//     function onAjaxSuccess(response) {
//         resetFormCompletely("crud_form"); // الفورم يتفرغ بالكامل
//         const modal = bootstrap.Modal.getInstance(
//             document.getElementById("crud_modal")
//         );
//         if (modal) modal.hide();
//         window.location.reload(); // الصفحة تتحمل بعد الإضافة
//     }
// });
