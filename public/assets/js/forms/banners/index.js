...
function retrieveBannersFormBackend() {
...
var bannerItems = function (response) {
    var banners = response.banners.data || {};
    var bannerCards = "";
    bannerCards = `
        <!--begin::Col-->
        <div class="col-md-5 col-xl-2 non-draggable">
            <!--begin::Card-->
            <div class="card" style="height: 100%;">
                <!--begin::Card body-->
                <div class="card-body d-flex flex-center flex-column" style="padding: 2rem 0;">
                    <!--begin::Button-->
                    <a href="#" class="btn btn-clear d-flex flex-column flex-center" id="add_btn" data-bs-toggle="modal" data-bs-target="#createBannerModal">
                        <!--begin::Illustration-->
                        <!--end::Illustration-->
                        <!--begin::Label-->
    ...
    if (Object.keys(banners).length > 0) {
        ...
        $.each(banners, function (index, banner) {
            bannerCards += `
                <div class="col-md-5 col-xl-2" id="card-${banner.id}">
                    <!--begin::Card-->
                    <div class="card border-hover-primary" style="height: 100%;overflow: hidden;">
                        <!--begin::Card header-->
                        <div class="card-header border-0 " style="padding: 0 0;position: relative;">
                            <!--begin::Card Title-->
                            <div class="card-title m-0 overflow-hidden">
                                <!--begin::Avatar-->
                                <div class="d-flex justify-content-center align-items-start" style="height: 150px;z-index: 1;">
                                    <div class="card-toolbar">
                                    <div class="me-0">
                                        <button type="button"
                                            class="btn btn-clean btn-sm btn-icon btn-icon-primary btn-active-light-primary dropdown-btn"
                                            ...
                                            <span class="svg-icon svg-icon-3 svg-icon-primary" style="color: #2b2928"><svg width="24" height="24"
                                                    viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path opacity="0.3"
                                        ...
                                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg-light-primary fw-semibold w-200px py-3"
                                            ...
                                            <div class="menu-item px-3">
                                                <a href="javascript:;"
                                                    id="edit" class="menu-link flex-stack px-3" data-kt-docs-table-filter="edit_row" data-id="${
                                                        banner.id
    ...
    <path d="M17.8591 10.4198C17.5891 10.2898 17.3291 10.1598 17.0891 10.0098C16.8891 9.88984 16.6891 9.75984 16.4991 9.61984C16.3391 9.51984 16.1591 9.36984 15.9791 9.21984C15.9591 9.20984 15.8991 9.15984 15.8191 9.07984C15.5091 8.82984 15.1791 8.48984 14.8691 8.11984C14.8491 8.09984 14.7891 8.03984 14.7391 7.94984C14.6391 7.83984 14.4891 7.64984 14.3591 7.43984C14.2491 7.29984 14.1191 7.09984 13.9991 6.88984C13.8491 6.63984 13.7191 6.38984 13.5991 6.12984C13.4691 5.84984 13.3691 5.58984 13.2791 5.33984L7.89912 10.7198C7.54912 11.0698 7.20912 11.7298 7.13912 12.2198L6.70912 15.1998C6.61912 15.8298 6.78912 16.4198 7.17912 16.8098C7.50912 17.1398 7.95912 17.3098 8.45912 17.3098C8.56912 17.3098 8.67912 17.2998 8.78912 17.2898L11.7591 16.8698C12.2491 16.7998 12.9091 16.4698 13.2591 16.1098L18.6391 10.7298C18.3891 10.6498 18.1391 10.5398 17.8591 10.4198Z" fill="currentColor"/>
                                            ...
                                            <!--begin::Menu item-->
                                            <div class="menu-item px-3">
                                                <a href="#" class="menu-link flex-stack px-3 hidden-item" data-id="${
                                                    banner.id
                                                }" data-hidden="${
                banner.hidden_flag == 1 ? 0 : 1
            }">${banner.hidden_flag == 1 ? __("Invisible") : __("Visible")}
    ...
    <svg xmlns="http://www.w3.org/2000/svg" id="Outline" viewBox="0 0 24 24" width="512" fill="currentColor" height="512">
        ...
        <path d="M12,7a5,5,0,1,0,5,5A5.006,5.006,0,0,0,12,7Zm0,8a3,3,0,1,1,3-3A3,3,0,0,1,12,15Z" />
                                            ...
                                            <!--begin::Menu item-->
                                            <div class="menu-item px-3">
                                                <a href="#" class="menu-link flex-stack px-3 delete-item" data-id="${
                                                    banner.id
                                                }">${__("Delete")}
        ...
        <path
            ...
            fill="currentColor" />
                                ...
                                <!--begin::Card toolbar-->
                                   <div class="rounded overflow-hidden w-100 position-absolute">
                                        <img src="${
                                            banner?.full_image_path
        ...
        if (response.total > 0) {
            // check if database contains banners
            bannerCards = ``;
            $("#no-results-alert").fadeIn();
        }
    }

    $(".banners-container").html(bannerCards);
    handleAddNew();
    handleEditRows();
    deleteBanner();
    hiddenBanner();
...
new Sortable(document.getElementById("sortable-banners"), {
    animation: 150,
    handle: ".card", // Optional: specify drag handle
    filter: ".non-draggable", // this makes the first card not draggable
    preventOnFilter: false, // allows interaction with content inside
    swapThreshold: 10,
    // invertSwap: true,
    ...
    onMove: function (evt) {
        ...
        // Prevent dropping before the first non-draggable card
        if (related.classList.contains("non-draggable")) {
            return false;
        }
    ...
    onEnd: function (/**Event*/ evt) {
        ...
        const sortedIds = [];
        document
            .querySelectorAll("#sortable-banners > div[id^='card-']")
            .forEach((el) => {
                sortedIds.push(el.id.replace("card-", ""));
            });

        // Optional: Send new order to backend
        // $.post('/dashboard/admin/products/sort', { ids: sortedIds });
        $.post("banners/sort", { ids: sortedIds });
    },
});
var paginator = function (response) {
    ...
    var banners = response.banners.data || [];
    var paginationData = response.banners;
    ...
    if (banners.length != 0) {
        ...
        var prevPageUrl =
            prevUrl !== "javascript:;"
                ? prevUrl + "&" + filterParams
                : "javascript:;";
        var nextPageUrl =
            nextUrl !== "javascript:;"
                ? nextUrl + "&" + filterParams
                : "javascript:;";
...
var handleAddNew = () => {
    $(document).ready(function () {
        $("#add_btn").click(function (e) {
            ...
            $("#form_title").text(__("Add new banner"));
            $("[name='_method']").remove();
            $(".image-input-wrapper").css(
                "background-image",
                `url('/placeholder_images/default.svg')`
            );
            $(".image-input-wrapper").css("background-position", `center`);
            $("#crud_form").trigger("reset");
            $("#crud_form").attr("action", `/dashboard/admin/banners`);
        });
    });
};
var handleEditRows = () => {
    ...
    editButtons.forEach((d) => {
        ...
        d.addEventListener("click", function (e) {
            e.preventDefault();
            const data = d.getAttribute("data-banner");
            const dataParse = JSON.parse(data);
            $("#form_title").text(__("Edit Banner"));
            // Fill modal fields
            $("#link_inp").val(dataParse.link ?? "");
            $("#expired_at_inp").val(dataParse.expired_at ?? "");
            $(".image-input-wrapper").css(
                "background-image",
                `url('${dataParse.full_image_path}')`
            );
            $(".image-input-wrapper").css("background-position", `center`);
            $("#crud_form").attr(
                "action",
                `/dashboard/admin/banners/${dataParse.id}`
            );
            $("#crud_form").prepend(
...
function deleteBanner(params) {
    $(".delete-item").on("click", function (e) {
        ...
        deleteElement("", `/dashboard/admin/banners/${id}`, () =>
            retrieveBannersFormBackend()
        );
    });
}

function hiddenBanner(params) {
    $(".hidden-item").on("click", function (e) {
        e.preventDefault();
        let id = $(this).attr("data-id");
        ...
        toggleBannerVisibility(
            "",
            `/dashboard/admin/banners/toggle-visibility/${id}`,
            hidden,
            () => retrieveBannersFormBackend()
        );
    });
