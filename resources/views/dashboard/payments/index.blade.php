...
<thead><tr class="fw-bold text-muted"><th>{{ __('الطالب') }}</th><th>{{ __('الشهر') }}</th><th>{{ __('المطلوب') }}</th><th>{{ __('المدفوع') }}</th><th>{{ __('المتبقي') }}</th><th>{{ __('الحالة') }}</th><th>{{ __('الطريقة') }}</th><th></th></tr></thead>
...
<td class="text-end">@if(in_array(auth('admin')->user()->type,['admin','teacher']))<a href="{{ route('admin.payments.edit',$p->id) }}" class="btn btn-sm btn-light-primary">{{ __('تحصيل') }}</a>@endif</td>
</tr>@empty<tr><td colspan="8" class="text-center text-muted py-10">{{ __('لا توجد فواتير') }}</td></tr>@endforelse</tbody>
</table></div></div></div>
