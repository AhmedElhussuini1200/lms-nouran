...
<h3 class="card-title">🔴 {{ $course->title }} @if($course->is_live)<span class="badge badge-light-danger">{{ __('مباشر الآن') }}</span>@endif</h3>
@if(in_array(auth('admin')->user()->type,['admin','teacher']))
<form method="POST" action="{{ route('admin.live.stop',$course->id) }}">@csrf<button class="btn btn-sm btn-danger">{{ __('إنهاء البث') }}</button></form>@endif</div>
<div class="card-body"><div id="chat" class="border rounded p-4 mb-3" style="height:300px;overflow-y:auto">
@foreach($messages as $m)<div><b>{{ $m->author->name ?? '' }}:</b> {{ $m->message }}</div>@endforeach</div>
<form id="chatform" class="d-flex gap-2"><input id="msg" class="form-control" placeholder="{{ __('اكتب رسالة...') }}" required>
<button class="btn btn-primary">{{ __('إرسال') }}</button></form></div></div>
<script>
...
function addMsg(m){let d=document.createElement('div');d.innerHTML='<b>'+(m.author?.name||'')+':</b> '+m.message;document.getElementById('chat').appendChild(d);}
