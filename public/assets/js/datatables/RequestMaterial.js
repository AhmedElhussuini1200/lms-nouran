...
var KTDatatablesServerSide = (function () {
    ...
    var initDatatable = function () {
        ...
        //                 // ملف العقد
        //                 const fileUrl = `/storage/WithdrawRequests/files/${row.file}`;
        //                 $("#current_contract_file").html(`
        ...
        //         },
                ...
                //                 !row.logs.is_approved_by_manager_contractor &&
                ...
                //             orderable: false,
                //             render: function (data, type, row) {
                //                 // جلب آخر لوج إذا موجود
                //                 let lastLog =
                //                     row.logs && row.logs.length
                //                         ? row.logs[row.logs.length - 1]
                //                         : null;

                //                 let approveRejectBtns = "";

                //                 // شرط ظهور أزرار الموافقة/الرفض
                //                 if (
                //                     userType1.roles?.some((role) => role.id === 1) && // هو المدير
                //                     (!lastLog ||
                //                         (!lastLog.is_approved_by_manager_contractor &&
                //                             !lastLog.rejected_by_manager_contractor)) &&
                //                     row.status?.name_en !==
                //                         "Rejected-by-Contractor-Manager"
                //                 ) {
                //                     approveRejectBtns = `
                //         <div class="menu-item px-3">
                //             <a href="javascript:;" class="menu-link px-3 text-success" data-kt-docs-table-filter="approve_row" data-id="${
                ...
                //         },
                {
                    ...
                    orderable: false,
                    render: function (data, type, row) {
                        // آخر تسجيل للمقاول (سواء وافق أو رفض)
                        let contractorLog = row.logs
                            ?.filter(
                                (l) =>
                                    l.is_approved_by_manager_contractor ||
                                    l.rejected_by_manager_contractor
                            )
                            .slice(-1)[0];
    ...
    var handleEditRows = () => {
        ...
        editButtons.forEach((d) => {
            d.addEventListener("click", function (e) {
                ...
                $("#quantity_inp").val(data.quantity).trigger("change");
