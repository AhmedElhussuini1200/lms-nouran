...
var KTDatatablesServerSide = (function () {
    ...
    var initDatatable = function () {
        datatable = $("#kt_datatable").DataTable({
            ...
            columnDefs: [
                ...
                //     render: function (data, type, row) {
                //         return `<div class="d-flex flex-column justify-content-center">
                //                     <a href="javascript:;" class="mb-1 text-gray-800 text-hover-primary">${row.name_en}</a>
                //                 </div>`;
                //     },
                // },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        <div>
                            <a href="#" class="btn btn-light btn-active-light-primary btn-sm " data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end" data-kt-menu-flip="top-end">
                                <span class="svg-icon svg-icon-dark svg-icon-1 m-0"><svg width="24" height="24" viewBox="0 0 24 24" fill="none">
    ...
    var handleEditRows = () => {
        ...
        editButtons.forEach((btn) => {
            btn.addEventListener("click", function (e) {
                ...
                // عنوان الفورم
                $("#form_title").text(__("Edit Priority"));

                // تعيين الحقول
                $("#name_ar_inp").val(data.name_ar);
                // $("#name_en_inp").val(data.name_en);

                // تعيين نوع الأولوية وإظهار الحقل
                const typeRadio = $(
                    `input[name="priority_type"][value="${data.priority_type}"]`
                );
                typeRadio.prop("checked", true);
                priorityField.handlePriorityTypeChange(data.priority_type);

                // تعبئة القيمة
                $("#priority_inp").val(data.dayPertirty);

                // اللون
                $("#color_inp").val(data.color);

                // تحديث الـ action للفورم مع PUT
                $("#crud_form").attr(
                    "action",
    ...
    //             $("#name_ar_inp").val(data.name_ar);
    //             // $("#name_en_inp").val(data.name_en);
    //             // $("#dayPertirty_inp").val(data.dayPertirty);
    //             $("#priority_inp").val(data.dayPertirty);
