...
var KTFileManagerList = (function () {
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row, meta) {
                        return `<span class="mb-1 text-gray-600">${
                            meta.row + 1
                        }</span>`;
    ...
    const handleDeleteRows = () => {
        ...
        deleteButtons.forEach((d) => {
            ...
            d.addEventListener("click", function (e) {
                ...
                Swal.fire({
                    text:
                        __("Are you sure you want to delete ") + fileName + "?",
                    icon: "warning",
                    ...
                    if (result.value) {
                        // Call DELETE API
                        const reportId = parent.getAttribute('data-report-id');
                        const link = e.target.closest('a');
                        const explicitUrl = link ? link.getAttribute('data-delete-url') : null;
                        const deleteUrl = explicitUrl || `/dashboard/consultant/reports/${reportId}`;
                        ...
                        fetch(deleteUrl, {
                            ...
                            .then(async (res) => {
                                const data = await res.json().catch(() => ({ message: __('Unexpected response') }));
                                if (res.ok) {
                                    Swal.fire({
                                        text: data.message || __("You have deleted ") + fileName + "!.",
                                        icon: "success",
                                        buttonsStyling: false,
                                    ...
                                    Swal.fire({
                                        text: data.message || __('Delete failed'),
                                        icon: "error",
                                        buttonsStyling: false,
                                        confirmButtonText: __("Ok, got it!"),
                                        customClass: {
                            ...
                            .catch(() => {
                                Swal.fire({
                                    text: __('Network error while deleting'),
                                    icon: "error",
                                    buttonsStyling: false,
                                    confirmButtonText: __("Ok, got it!"),
                                    customClass: {
        ...
        deleteSelected.addEventListener("click", function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    Swal.fire({
                        text: __("Selected  files or folders was not deleted."),
                        icon: "error",
                        buttonsStyling: false,
                        confirmButtonText: __("Ok, got it!"),
                        customClass: {
            ...
            if (renameValidator) {
                renameValidator.validate().then(function (status) {
                    ...
                    if (status == "Valid") {
                        ...
                        Swal.fire({
                            text:
                                __("Are you sure you want to rename ") +
                                nameValue +
                                "?",
                            icon: "warning",
                            showCancelButton: true,
                            ...
                            if (result.value) {
                                ...
                                Swal.fire({
                                    text: nameValue + __(" was not renamed."),
                                    icon: "error",
                                    buttonsStyling: false,
                                    confirmButtonText: __("Ok, got it!"),
                                    customClass: {
    ...
    //             url: "dashboard/consultant/store", // Set the url for your upload script location
    //             paramName: "files[]",
    //             parallelUploads: 10,
    ...
    //             params: {
    //                 report_id: reportId // <-- ضع هنا الـ report_id المناسب
    //             }
    //         });
    ...
    const initDropzone = () => {
        ...
        var myDropzone = new Dropzone(id, {
            // Resource route: POST /dashboard/consultant/reports
            url: "/dashboard/consultant/reports", // Laravel resource store endpoint
            paramName: "files[]",
            parallelUploads: 10,
            previewTemplate: previewTemplate,
            acceptedFiles: ".pdf",
            ...
            dictFileTooBig:
                __("The file is too large ({{filesize}} MB). Maximum allowed size: {{maxFilesize}} MB."),
            dictMaxFilesExceeded:
                __("You cannot upload more than {{maxFiles}} file(s)."),
            dictFallbackMessage: __("Your browser does not support drag-and-drop file uploads."),
            dictInvalidFileType: __("You cannot upload files of this type."),
            dictResponseError: __("The server responded with status code {{statusCode}}."),
            params: {
                report_id: reportId, // must be defined globally in Blade
            },
            headers: {
        ...
        // 🔹 When all uploads complete: show success only if no errors
        myDropzone.on("queuecomplete", function () {
            const rejected =
                typeof myDropzone.getRejectedFiles === "function"
                    ? myDropzone.getRejectedFiles()
                    : [];
            const hasError =
                rejected.length > 0 ||
                myDropzone.files.some((f) => f.status === "error");

            // Helper to reload the page so the datatable reflects new files
            const reloadPage = () => {
            ...
            if (!hasError) {
                Swal.fire({
                    text: __("All files uploaded successfully!"),
                    icon: "success",
        ...
        dropzone
            ...
            .addEventListener("click", function () {
                Swal.fire({
                    text: __("Are you sure you want to remove all files?"),
                    icon: "warning",
                    showCancelButton: true,
                    buttonsStyling: false,
