...
@section('content')
<div class="card"><div class="card-header"><h3 class="card-title fw-bold">{{ __('إضافة واجب') }}</h3><div class="card-toolbar"><a href="{{ route('admin.assignments.index') }}" class="btn btn-light">{{ __('رجوع') }}</a></div></div>
<form action="{{ route('admin.assignments.store') }}" method="POST" enctype="multipart/form-data" class="ajax-form" data-success-callback="onAjaxSuccess">@csrf
...
<div class="col-md-4"><label class="form-label">{{ __('تاريخ التسليم') }}</label><input type="date" name="due_date" class="form-control" /></div>
...
<div class="col-md-4"><label class="form-label">{{ __('ملف مرفق') }}</label><input type="file" name="file" class="form-control" /></div>
<div class="col-12"><label class="form-label">{{ __('الوصف') }}</label><textarea name="description" class="form-control" rows="4"></textarea></div>
</div></div>
