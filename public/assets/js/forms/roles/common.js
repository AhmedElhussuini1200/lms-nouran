...
$(".edit-rank-btn").click(function () {
    ...
    $.ajax({
        url: "/dashboard/admin/ranks/" + rankId + "/edit",
        method: "GET",
        success: function (response) {
            ...
            $("#type_inp").val(response["type"]);
            $("#completed_orders_count_inp_edit").val(
                response["completed_orders_count"]
            );
            $("#commission_inp_edit").val(response["commission"]);

            // set the route to the update form
            $("#rank_form_update").attr(
                "action",
                `/dashboard/admin/ranks/${rankId}`
            );
        },
    });
});
...
$(".edit-role-btn").click(function () {
    ...
    $.ajax({
        url: "/dashboard/admin/settings/roles/" + roleId,
        method: "GET",
        success: function (response) {
            console.log(response["name_ar"]);
            ...
            // set the route to the update form
            $("#role_form_update").attr(
                "action",
                `/dashboard/admin/settings/roles/${roleId}`
            );
